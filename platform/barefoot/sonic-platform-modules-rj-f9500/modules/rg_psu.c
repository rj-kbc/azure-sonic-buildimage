 ...
 *
 * Copyright (c) 2018 tjm < tangjiamiao@ruijie.com.cn>
 * 
 */
...
static ssize_t show_psu_value(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    ret = i2c_smbus_read_i2c_block_data(client, attr->index, 2, psu_buf);
    if (ret < 0) {
...
static ssize_t show_psu_sysfs_value(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    mutex_lock(&data->update_lock);
    DBG_DEBUG("ret:%d", ret);
    ret = i2c_smbus_read_i2c_block_data(client, attr->index, 2, smbud_buf);
    if (ret < 0) {
		DBG_ERROR("Failed to read psu \n");
	}
    value = smbud_buf[1];
    ...
    return snprintf(buf, PSU_SIZE,  "%ld\n", result);
}
...
static ssize_t show_sysfs_15_value(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    memset(smbud_buf, 0, PSU_SIZE);
    mutex_lock(&data->update_lock);
    ret = i2c_smbus_read_i2c_block_data(client, attr->index, 15, smbud_buf);
    if (ret < 0) {
        DBG_ERROR("Failed to read psu \n");
    }
    mutex_unlock(&data->update_lock);
